...
- Order dari kasir: `source = 'kasir'`. Status OTOMATIS `selesai` HANYA kalau bukti transaksi ada di pembayaran awal; tanpa bukti, masuk sebagai `baru`. Tombol status "Selesai" di Kelola Pesanan dikunci (disabled + tooltip) untuk order kasir yang belum SEMUA cicilannya punya bukti -- lihat `needsProofBeforeComplete()` di Orders.jsx (detail cicilan & multi-bukti: lihat bagian "Riwayat Pembayaran" di bawah). Bukti bisa diupload belakangan per cicilan langsung dari situ (ordersApi.attachPaymentProof()) kalau kelewat pas transaksi awal.
...
- Syarat status Selesai diperketat: SEMUA baris `order_payments` pesanan itu (bukan cuma salah satu cicilan) harus punya `proof_path` -- `needsProofBeforeComplete()` di Orders.jsx sekarang cek `order.order_payments` langsung, bukan cache `orders.proof_path`. Dikunci juga di database lewat trigger `prevent_status_change_by_non_admin()` (perbaikan-lock-selesai-bukti.sql), jadi gak bisa dilewatin lewat panggilan API langsung ke `updateOrderStatus()`.
- Migrasi SQL: migrasi-multi-payment.sql (root project), termasuk backfill order kasir lama jadi satu baris `order_payments` masing-masing. Perbaikan syarat Selesai di atas: perbaikan-lock-selesai-bukti.sql (jalankan setelah migrasi utama).
